  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Plan Types</h1>
          <p className="text-slate-600 mt-2">Manage the plan types used for insurance categorization</p>
        </div>
      ...
      {showCreateForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-8 w-full max-w-md shadow-2xl">
            <h2 className="text-2xl font-bold text-slate-800 mb-6">Create New Plan Type</h2>
            <div className="space-y-6">
              ...
              <div className="flex items-center">
                <input
                  ...
                  className="mr-3 w-4 h-4 text-emerald-600 bg-slate-100 border-slate-300 rounded focus:ring-emerald-500"
                />
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-lg overflow-hidden">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-200">
              {paginatedPlanTypes.map((planType) => (
                <tr key={planType.id} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4">
                    {editingPlanType === planType.id ? (
                      <input
                        ...
                        className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200"
                      />
                    ) : (
                      <span className="font-semibold text-slate-800">{planType.display_name}</span>
                    )}
                  </td>
                  <td className="px-6 py-4">
                    {editingPlanType === planType.id ? (
                      ...
                      <span className="text-slate-600 text-sm">{planType.description || '-'}</span>
                    )}
                  </td>
                  <td className="px-6 py-4">
                    {editingPlanType === planType.id ? (
        ...
        {planTypes.length === 0 && (
          <div className="text-center py-12">
            <p className="text-slate-500">No plan types found.</p>
            <button
              onClick={() => setShowCreateForm(true)}
