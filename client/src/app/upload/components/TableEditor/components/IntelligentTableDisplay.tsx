...
const IntelligentTableDisplay: React.FC<IntelligentTableDisplayProps> = ({ extractionResult }) => {
  const { document_metadata, tables, extraction_quality, extraction_intelligence } = extractionResult
...
const DocumentMetadataSection: React.FC<{ metadata: DocumentMetadata, confidence?: number }> = ({ metadata, confidence }) => {
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2">
          ...
          <div className="pl-6">
            <p className="text-sm text-gray-900 font-medium">
              {metadata.carrier_name || 'Not identified'}
            </p>
        ...
        <div className="space-y-2">
          ...
          <div className="pl-6">
            <p className="text-sm text-gray-900 font-medium">
              {metadata.statement_date || 'Not identified'}
            </p>
          ...
          tables.map((table, index) => (
            <div key={index} className="border border-gray-200 rounded-lg p-4">
              ...
              {table.rows && table.rows.length > 0 && (
                <div className="text-xs text-gray-500">
                  Sample data: {table.rows.slice(0, 2).map((row: any[]) => 
                    row.slice(0, 3).join(', ')
                  ).join(' | ')}
                  {table.rows.length > 2 && '...'}
                </div>
    ...
    <div className="bg-white rounded-lg border border-gray-200 p-6">
      ...
      {intelligence && (
        <div className="mt-4 pt-4 border-t border-gray-200">
          <div className="text-xs text-gray-500">
            <div>Method: {intelligence.analysis_method}</div>
            <div>Version: {intelligence.intelligence_version}</div>
