#!/usr/bin/env python3
"""
Test the original working approach to verify it still works
"""
...
def test_original_approach():
    ...
    pdf_path = "pdfs/test.pdf"  # You'll need to provide a test PDF
    ...
    if os.path.exists(pdf_path):
        ...
        print("No test PDF found. Testing header detection logic only.")
        
        # Test header detection with sample data
        ...
        print(f"Header row: {header_likely(header_row)}")
        print(f"Data row: {header_likely(data_row)}")
        
        print(f"\nTesting number detection:")
        for i, cell in enumerate(data_row):
            print(f"Cell {i}: '{cell}' - Is number: {is_number(cell)}")
